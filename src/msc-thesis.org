...
** WAIT The Dust Particle Mass Distribution Function
- Continuous
...
\newpage
\newpage
** WAIT The Smoluchovski Coagulation Equation
...
** WAIT +Introduction/General Idea+
...
coagulation kernel.
** TODO Discretization of the Mass Axis
*** +Introduction/General Idea+
We will start with the discretization of the mass axis: Here, the goal is to partition the
continuous range of masses present in the disk into a set of $\mathcal N_m\in\mathbb N$
...
value $m_i^\text{c}$, situated at the center of the bin (ergo the superfix "c").
...
\begin{figure}[h!]
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \def\R{\W/16}  % This is the padding between arrow & text.
            ...
            \node[] at (0, \H+\L+\P+\L+\R+\L+\L) {$m_\text{min}$};
            \node[] at (\N*\W+3*\W, \H+\L+\P+\L+\R+\L+\L) {$m_\text{max}$};
            \node[] at (0, \H+\L+\P+\L+\R+\L) {$=$};
            \node[] at (\N*\W+3*\W, \H+\L+\P+\L+\R+\L) {$=$};
        \end{tikzpicture}
    \end{center}
\end{figure}
...
Analogously to the bin centers, the boundary points can be labeled by an index $i\in[0,\mathcal N_m]$. The corresponding mass value $m_i^\text{b}$ can be expressed as
\begin{align*}
...
\begin{align*}
  \Delta m=m_{i+1}^\text{b}-m_i^\text{b}=\frac{m_\text{max}-m_\text{min}}{\mathcal N_m}
\end{align*}
\newpage
...
** WAIT Discretization of the Particle Mass Distribution Function
...
\end{align*}
** WAIT Discretization of the Smoluchovski Coagulation Equation
...
\newpage
** WAIT Initialization of the Particle Mass Distribution Function
...
\newpage
** WAIT Integration of the Smoluchovski Equation
...
** WAIT Introduction, Overview, & Goals for this Section
...
\newpage
** WAIT Definition of the Kernel for simple Stick-and-Hit Coagulation
...
\newpage
** WAIT Numerical Integration with the Explicit Euler Scheme
...
** WAIT Introduction, Overview, & Goals for this Section
** WAIT Reasons for Making Use of a Logarithmic Representation
** WAIT Challenges Associated with a Logarithmic Representation
** WAIT Definition of the Logarithmic Mass Axis
*** QUES How to discretize the mass axis in a logarithmic fashion?
\newpage
** WAIT Implementation of the Kovetz-Olund algortihm
...
\newpage
** WAIT Implementation of Near-Zero-Cancellation Handling for Coagulation
...
** WAIT "First/Simple/Naive Attempt"
*** QUES How to implement naive fragmentation?
- "Pulverization"
\newpage
** WAIT The MRN Distribution
...
** WAIT "Implementation of Near-Zero-Cancellation Handling for Fragmentation" (?)
\newpage
* Definition of a the Dust Particle Reaction Rates
** WAIT Definition of the Dust Particle Reaction Rates
\newpage
** WAIT Definition of the Dust Particle Collision Rates
...
\newpage
** WAIT Definition of the Collision Cross Section
...
\newpage
** WAIT Definition of the Relative Particle Velocity
...
** WAIT Definition of the Relative Velocity due to Radial Drift
\begin{align*}
...
\newpage
** WAIT Definition of the Relative Velocity due to Differential Settling
\begin{align*}
...
\newpage
** WAIT Definition of the Relative Velocity due to Brownian Motion
\begin{align*}
  \Delta u^\text{BR}_{ij}
...
\newpage
** WAIT Definition of the Relative Velocity due to Turbulence
#+ATTR_LATEX: :width \textwidth
[[../../msc-thesis.py/figures/22/dv_TU.pdf]]
\newpage
** WAIT Definition of the Azimuthal Relative Velocity
\begin{align*}
  \Delta u_{ij}
  =\left|u_n\cdot\left(
