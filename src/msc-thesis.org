...
#+startup: latexpreview show1level
#+author: Vincent C. Mader
#+title: Computational Cost Optimization for Numerical Integration of the Smoluchovski Equation via Stochastic Sampling of the Coagulation Kernel
#+latex: \listoffigures
#+latex: \listoftables
\newpage
* Preface
...
\newpage
* Introduction
** "As of yet unsorted Things to put into the Introduction"
*** QUES What even is coagulation?
...
*** QUES What is the interstellar medium composed of?
...
*** QUES How do protoplanetary disks form?
...
*** QUES Why does a star form at the center of the disk?
- gravity $\to$ high pressure $\to$ nuclear fusion
*** QUES What does a protoplanetary disk look like?
...
** "Particle Kinematics inside the Disk"
...
*** QUES How do collisions lead to the formation of planetary cores?
...
*** QUES What is the structure of this thesis?
- ...
**** Introduction
**** Attempt A
***** "Stick-and-Hit Coagulation"
***** "Initialization of the Dust Particle Mass Distribution Function"
***** "Implementation of Physical Reaction Rates"
****** "Implementation of Physical Collision Rates"
******* "Implementation of Physical Collision Cross Section"
******* "Implementation of Physical Relative Velocities"
****** "..."
**** Attempt B
- Give context/introduction.
  1. What is the goal?
  2. (?) How do planets form?
  3. How do PPDs form?
  4. What makes up a disk?
  5. What does a disk "look" like?
     - radially
  6. .........

- Implement a first hands-on approach of what we're even talking about:
  1. How do we =discretize= the particle mass distribution function?
     - Here: Do it on a =linear= grid.
  2. How do we discretize the Smoluchovski equation?
  3. How do we define a (simple/naive) coagulation kernel?
     - Here: Include only =stick-and-hit= coagulation.
     - Here: Set reaction rates to 1.
  4. How do we define the initial particle mass distribution?
     - Here: Use Dirac-delta distribution.
  5. How to we =integrate= the Smoluchovski equation?
     - Here: Do it =explicitly=.

- Move to a =logarithmic= representation.
  1. How do you define a logarithmic (discrete) mass axis?
     - Why even do this? -> Talk about involved scales.
     - What are the problems here?
       + Talk about resulting masses not lying on grid points.
       + Talk about near-zero cancellation.
  2. How do we fix the grid-point problem?
     - Implement =Kovetz-Olund= algortihm.
  3. How do we fix the cancellation problem?
     - Implement =cancellation handling=.

- Move away from explicit integration, use =implicit integration=.
  1. Implement =logarithmic time axis=.
  2. ...

- Include =fragmentation= into the model.
  1. What is a first attempt at doing this?
     - Move mass into the smallest-mass bin.
  2. How can one do this more accurately?
     - Implement MRN distribution.
  3. (?) Implement cancellation handling.

- (?) Implement initial distribution using MRN
  1. ...

- Implement =physical reaction rates=.
  1. What are the reaction rates made up of?
     - ...
  2. What are the collision rates made up of?
     - Define collision rate from cross section & rel. velocity.
  3. How is the collision cross section defined?
     - Define cross section.
  4. How are the relative velocities defined?
     - Define total rel. velocity.

- (?) Implement =physical gas model= (radial).           <- Do this here?
  1. ...

- Implement =physical relative velocites=.
  1. ... (RD, AZ, DS, TU)

- Implement =stochastic sampling= of the kernel.
  1. ...

     \newpage
* Definition of a Disk Model
** "Notes on what we plan to do in this section"
...
* "First/Basic/Naive Implementation"
** "Notes on what we plan to do in this section"
...
** Discretization & Initialization of the Particle Mass Distribution Function
...
** "Implicit Solvers"
\newpage
* Implementation of a Logarithmic Representation for the Mass Axis
...
4. The second condition is that of mass conservation, which can be written as:
   ...
   \end{align*}
5. Now, for the splitting of the resulting particle's mass onto two bins, let
   us define the parameter $\varepsilon$ such that
   \begin{align*}
     K_{\bar k,ij}^\text{gain}
...
\newpage
* Inclusion of Fragmentation Processes into the Model
...
\newpage
* Implementation of a more Physical Model for the Dust in the PPD
...
\newpage
* "Implementation of a more Physical Model for the Gas in the PPD" (?)
\newpage
* Implementation of a more Physical Model for the Relative Particle Velocities
...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../figures/22/dv_AZ.pdf}
  \caption{1a}
  \label{fig:sfig1}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../figures/22/dv_TU.pdf}
  \caption{1b}
  \label{fig:sfig2}
\end{subfigure}
\caption{plots of....}
...
[[../figures/01/discrete-mass-axis_lin-scale.pdf]]
\newpage
...
#+ATTR_LATEX: :width \textwidth
[[../figures/01/discrete-mass-axis_log-scale.pdf]]
\newpage
...
\end{align*}
#+ATTR_LATEX: :width \textwidth
[[../figures/02/disk_scale_height.pdf]]
\newpage
*** Kepler Frequency
Kepler velocity:
\begin{align*}
...
\end{align*}
#+ATTR_LATEX: :width \textwidth
[[../figures/02/kepler_frequency.pdf]]
\newpage
...
\end{align*}
#+ATTR_LATEX: :width \textwidth
[[../figures/02/midplane_temperature.pdf]]
\newpage
**** Derivation
...
\end{align*}
#+ATTR_LATEX: :width \textwidth
[[../figures/02/gas_surface_density.pdf]]
\newpage
*** Gas Volume Density
\begin{align*}
  \rho_g
...
\end{align*}
#+ATTR_LATEX: :width \textwidth
[[../figures/02/gas_volume_density.pdf]]
\newpage
...
#+ATTR_LATEX: :width \textwidth
[[../figures/02/pressure_gradient.pdf]]
\newpage
...
*** Stokes Number
#+ATTR_LATEX: :width \textwidth
[[../figures/03/stokes_number.pdf]]
\newpage
*** Reynold's Number
#+ATTR_LATEX: :width \textwidth
[[../figures/03/reynolds_number.pdf]]
\newpage
*** Stopping Time
...
\end{align*}
\newpage
*** Particle Kinematics in the Disk
**** Brownian Motion
\newpage
**** Radial Drift
\newpage
...
\end{align*}
#+ATTR_LATEX: :width \textwidth
[[../figures/22/dv_RD.pdf]]
\newpage
**** Azimuthal Relative Velocity
\begin{align*}
  \Delta u_{ij}
  =\left|u_n\cdot\left(
...
\end{align*}
#+ATTR_LATEX: :width \textwidth
[[../figures/22/dv_AZ.pdf]]
\newpage
**** Turbulent Motion
#+ATTR_LATEX: :width \textwidth
[[../figures/22/dv_TU.pdf]]
\newpage
...
\end{align*}
#+ATTR_LATEX: :width \textwidth
[[../figures/22/dv_tot.pdf]]
\newpage
*** Particle-Particle Collisions
**** Introduction
In a two-body collision, we label the particles' masses $m_1$ and $m_2$,
and their velocities $\vec v_1$ and $\vec v_2$. The relative velocity is then given
by the difference $\Delta\vec v:=\vec v_2-\vec v_1$. For its absolute value we define
the shorthand notation $\Delta v:=|\Delta\vec v|=|\vec v_2-\vec v_1|$.

The probability of more than two particles colliding
at the same time is so tiny that three-body-collisions
can be neglected entirely [citation needed].
**** Collision Outcomes
- What outcomes are there?
  1. Bouncing
  2. Merging
  3. Fragmentation
- What happens in bouncing?
  + ...
- What happens in merging?
  + ...
...
\begin{align*}
  a_i\sim m_i^{1/3}
\end{align*}
[[../figures/21/collision_cross_section.pdf]]
\newpage
**** Collision Rate
\begin{align*}
  C_{ij}=\sigma_{ij}\cdot\langle\Delta v\rangle
\end{align*}
[[../figures/23/collision_rate.pdf]]
\newpage
...
\newpage
*** Introduction
The kernel $K_{kij}$ is approximately equal to the
right-hand side of the following statement:
\begin{align*}
  K_{kij}\approx(m_{k+1/2}-m_{k-1/2})\cdot K(m_k,m_i,m_j)
\end{align*}
[explain why]

Here, $m_i=(m_{i+1/2}+m_{i-1/2})/2$ (same for $j$ and $k$).

The discrete kernel is a 3D matrix of dimension $\mathcal N_m\times\mathcal N_m\times\mathcal N_m$.
[why]
...
*** Gain & Loss Term
_Inserting the kernel into the Smoluchovski equation_
- we see: there is a positive term and a negative term (gain & loss)
- why loss term? $\to$ when two particles merge, the original particles are no
  longer there
- why gain term? $\to$ when two particles merge, a new particle emerges
  [...]
  [reformulate]
  [generalize to include fragmentation]
  [add equation 1.11 from "Dust Evolution with Binning Methods"]
*** The Kovetz-Olund algorithm

The two points outlined above are addressed by an algorithm
outlined by [cite: Kovetz & Olund, 1969].

To solve the issues at hand by separating the kernel into two parts:
\begin{align*}
  \pderiv{N_k}{t}
  =\sum_{i,j}K^\text{gain}_{kij}N_iN_j
  -\sum_{j}K^\text{loss}_{kj}N_kN_j
\end{align*}
This is equivalent to the above-stated formulation of the
discrete Smoluchovski equations, if
\begin{align*}
  K_{kij}=K^\text{gain}_{kij}-K^\text{loss}_{kij}\cdot\delta_{ki}
\end{align*}
Here, $\delta_{ki}$ labels the Kronecker-delta.

This approach is suitable for both hit-and-stick coagulation as
well as when fragmentation processes are included into the model.

For the simpler hit-and-stick case:
- Let $(i, j)$ be a pair of colliding (& merging) particles.
# - The resulting mass $m_\text{res}=m_i+m_j$ lies somewhere between two
#   points in the grid, which we label using the indices $\bar k$ and $\bar k+1$,
#   such that
- Each collision of two particles results in exactly one new
  particle. This condition can be expressed as
\begin{align*}
  K^\text{gain}_{\bar k,ij}
  +K^\text{gain}_{\bar k+1,ij}
  =K^\text{loss}_{\bar ij}
\end{align*}
- Mass conservation
\begin{align*}
  m_{\bar k}K^\text{gain}_{\bar k,ij}
  +m_{\bar k+1}K^\text{gain}_{\bar k+1,ij}
  =(m_i+m_j)\cdot K^\text{loss}_ij
\end{align*}
- Linear ansatz for distributing the $m_\text{res}$ onto the two bins $\bar k$ and $\bar k+1$:
\begin{align*}
  K^\text{gain}_{\bar k,ij}
  &=(1-\varepsilon)\cdot K^\text{loss}_{ij}\\
  K^\text{gain}_{\bar k+1,ij}
  &=\varepsilon\cdot K^\text{loss}_{ij}
\end{align*}
- This leads to
\begin{align*}
  \varepsilon
...
# One possibiltiy for dividing the mass between the bins $k$ and $k+1$
# is given by the following linear ansatz:
# \begin{align*}
#   Q_m&=\varepsilon\cdot Q_{ij}\\
#   Q_n&=(1-\varepsilon)\cdot Q_{ij}
# \end{align*}
# [Dullemond & Brauer 2007]
# - Here: Number density $n$ conserved, since $Q_m+Q_n=Q_{ij}$.
# - More important though: Conservation of mass!
#   + enforce conservation via
#     $$Q_mm_m+Q_nm_n=Q_{ij}(m_i+m_j)$$
#   + this defines the (until now free) parameter $\varepsilon$:
#     $$\varepsilon=\frac{m_n-(m_i+m_j)}{m_n-m_n}$$
# - This can be used to rewrite the coagulation equation as
#     $$N_k=\frac{1}{2}\sum_{ij}Q_{ij}C_{ijk}-\sum_iQ_{ik}$$
#   with
# \begin{equation*}
#   C_{ijk}=
#   \begin{cases}
#     \varepsilon& \text{if $m_k$ is the largest grid point $<m_i+m_j$}\\
#     1-\varepsilon& \text{if $m_k$ is the smallest grid point $>m_i+m_j$}\\
#     0& \text{else}\\
#   \end{cases}
#    \end{equation*}
...
**** "Cases where near-zero cancellation occurs" [rename section]
- Collision events between particles of very different sizes/masses,
  e.g. very small mass $m_i$, very big mass $m_j$ fuse.
- Resulting mass $m_{\bar k}$ differs only by a tiny bit from $m_j$
\begin{align*}
  m_{\bar k}=m_i+m_j\approx m_j
\end{align*}
- The new particle will be assigned to "the same bin"
\begin{align*}
  \bar k=j
\end{align*}
- Here, $\varepsilon\ll1$. [explain why]
- Smoluchovski eq.:
\begin{align*}
  \pderiv{N_{\text k}}{t}
  &=K^\text{gain}_{\bar k,i\bar k}N_iN_{\bar k}-K^\text{loss}N_iN_{\bar k}\\
  \pderiv{N_{\text k+1}}{t}
  &=K^\text{gain}_{\bar k+1,i\bar k}N_iN_{\bar k}\\
\end{align*}
- Loss term and gain term related by: [link to eq. from K.O. algorithm]
\begin{align*}
  K^\text{gain}_{\bar k,ij}
  &=(1-\varepsilon)\cdot K^\text{loss}_{ij}
\end{align*}
...
\begin{align*}
  \pderiv{N_{\text k}}{t}
  &=(1-\varepsilon)\cdot K^\text{loss}_{\bar k,i\bar k}N_iN_{\bar k}-K^\text{loss}N_iN_{\bar k}\\
\end{align*}
- For $\varepsilon<\varepsilon_m$ (machine precision), this will not give accurate results.
**** Solution
- Identify cases where method breaks down,
- then carry out the cancellation analytically.

_"Undangerous" cases_:
All pairs $i,\ j$ for which $\bar k>\max(i,\ j)$
- Here, simply use $K^\text{gain}_{\bar k,ij}$ and $K^\text{loss}_{\bar k,ij}$ (no risk of near-zero cancellation).

"Dangerous" cases:
All pairs $i,\ j$ for which $\bar k=max(i,\ j)$
- Here, use $-\varepsilon K^\text{loss}_{\bar k,\min(i,j)}$
\newpage
*** QUES Where to talk about discretization of the radial axis?
*** QUES Where to talk about Stokes?
**** Stokes' Law
The force of viscosity on a small sphere
moving through a viscous fluid is given by
  $$F_d=6\pi\mu Rv$$
# Assumptions:
# - Laminar flow
# - Spherical particles
...
**** Stokes' Number
= the ratio of the characteristic time of a particle
  to the characteristic time of the flow (or of an obstacle).
  $$\text{St}=\frac{t_0\ u_0}{l_0}=\frac{\tau_\text s}{\tau_\text{ed}}$$
  where $\tau_\text{ed}$ is the eddy turn-over time and $\tau_\text s$ is the stopping time
**** Stokes' Regime
= the regime where the drag force on a
particle is described by the Stokes law.
+ This is not directly related to the Stokes number.
*** QUES Where to talk about the Toomre parameter?
**** Toomre parameter
*** General
_TODOs_:
- Auf Fehler/Tuecken eingehen, Tipps geben.
- Logarithmische Zeit-Skala einfuehren.
- Beispiel mit linearem Mass Grid machen.
- Stability Analysis machen, Limits testen.
- Do local & global disk simulations.
- Importance kernel sampling machen.
- Vergleiche
  + linear/logarithmic
  + near-zero cancellation handling on/off
  + cross section on/off
  + relative velocities on/off (each)
  + Kovetz-Olund on/off
- Compare performance at the end.
  + sampling vs. no sampling
  + ? parallelization vs. no parallelization
  + ? rust vs. python
- Talk about kernel variants:
  + constant
  + linear
  + quadratic
*** PPD Formation
...
_Processes_:
 1. Newtonian gravitational n-body interaction\\
    $\Rightarrow$ Keplerian motion (+gas pressure)
 2. Particle Collisions\\
    $\Rightarrow$ Angular momentum averages out.\\
    $\Rightarrow$ Cloud flattens out into a disk
       (oriented perpendicular to total angular momentum).
 3. Merge events ("coagulation").\\
    $\Rightarrow$ Larger bodies form (slowly, over many orders of magnitude).\\
    $\Rightarrow$ This eventually leads to the formation of planetary cores.
\newpage
*** Disk Model
**** Introduction
What do we need to define?
...
*** Particle Kinematics
**** Introduction
_Two competing motions_:
1. Settling towards the mid-plane.
2. Turbulent stirring back up again. [?]\\

_Velocity profiles_:
- For pairs of masses, the relative velocities
  follow a probability density function $\pi(\Delta v)$.
- Relative speed differences stem from:
  + Brownian motion,
  + differential settling,
  + relative motions caused by turbulence,
  + etc. (above are most important).
- If the relative velocities are due to
  + turbulent stirring: $\to$ Maxwell-Boltzmann distribution.
  + systematic differential drift: $\to$ Dirac $\delta_D$ function.\\

_In absence of coagulation_:
 - Eventually, equilibrium is reached.
 - Then, the upper layers of disk are devoid of dust of particular size.
 - Below a certain $Z$, the grains are fully mixed with the gas.
   + This leads to a more or less constant abundance.\\
     (e.g. Dubrulle et al. 1995; Takeuchi & Lin 2002)\\

Thus, the motion of particles in the disk is partly systematic
(driven by e.g. Keplerian forces), and partly random (e.g. thermal
motion). [...]

This leads to occasional particle collision events.
\newpage
**** Particle Collisions
***** Merging Probability
In reality, not every collision leads to a merge event. The probability
that it does, i.e. the merging probability $P(m_1,m_2,\Delta v)$, depends not
only on the particle masses $m_1$ and $m_2$, but also on the absolute value
of their relative velocity $\Delta v$.

If the relative velocity of two colliding particles is low, this usually
leads to a merge event [citation needed]. For high values of $\Delta v$ on the
other hand, this assumption can not be made [citation needed]. Here,
the particles either bounce, which does not change the particle mass
distribution at all, or they undergo fragmentation. [...]

The merging probability, also called "sticking coefficient" has to be
measured empirically. [...]
***** Collision-and-Merging Rate

The rate of collision events that leading to a merging of the two colliding
particles depends on both the collision rate $C(m_1,m_2)$, as well as on the
merging probability $P(m_1,m_2,\Delta v)$. It can be written as
\begin{align*}
  R(m_1,m_2)
  =\int_0^\infty C(m_1,m_2,\Delta v)\cdot P(m_1,m_2,\Delta v)\ \dd\Delta v
\end{align*}
If every collision leads to merging, then $R=C$ (hit-and-stick case).
the collision-and-merging rate $R$ is equal to the collision rate.

Since the inclusion of velocity dispersion into the calculation
of $R$ leads to an increased computational cost, a simplification
can be made by using the mean value of the relative velocity $\langle\Delta v\rangle$
instead. This leads to the (mostly sufficiently [citation needed])
approximate expression
\begin{align*}
  R(m_1,m_2)\approx C(m_1,m_2,\langle\Delta v\rangle)\cdot P(m_1,m_2,\langle\Delta v\rangle)
\end{align*}

# "This simplified form of $R(m_1, m_2)$ separates the collision rate from
# the collision out- come. It allows us to first estimate how often a
# collision takes place, and then worry about what happens next. This
# is the form which most coagulation papers in astrophysics use." [reformulate]
\newpage
...
* QUESTIONS
** QUES Hat es einen bestimmten Grund, dass Du in "Dust Evolution with Binning Methods" immer partielle statt totale zeitliche Ableitungen der Teilchen-Massen-Distributions-Funktion verwendest?
** QUES Wie /genau/ bastel ich mir meine Bins?
Genauer?
- Wo sollte ich die =Grid Cell Centers=,
  und wo die =Grid Cell Boundaries= benutzen?

Zwischen den beiden habe ich vermutlich bis jetzt
noch nicht konsequent genug differenziert.

Vermutlich:
- Die Centers ueberall dort, wo ein $m_k$ vorkommt.
- Die Boundaries ausschliesslich zur Berechnung von $\Delta m_k$.
