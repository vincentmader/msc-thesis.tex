...
continuous range of masses present in the disk into a set of $\mathcal N_m\in\mathbb N$ intervals, which
from now on will be referred to as mass grid "bins". Each of these bins can be uniquely
characterized by an index $i\in[0,\mathcal N_m-1]$, and its corresponding mass value $m_i^\text{c}$,
situated at the center of the bin (ergo the superfix "c").

To derive an expression for these mass values, let us first define the upper and lower
boundaries of the discrete mass grid. We shall label them $m_\text{max}$ and $m_\text{min}$, respectively.
Now, consider a collection of $\mathcal N_m+1$ appropriately spaced grid points, which will serve as
the bin boundaries. The spacing of these grid points of course depends heavily on the utilized
...
Here, the corresponding mass value is $m_i^\text{b}$ (with the superscript short for "boundary").
It can be expressed as
\begin{align*}
  m_i^\text{b}=m_\text{min}+(m_\text{max}-m_\text{min})\cdot\frac{i}{\mathcal N_m}
\end{align*}

To calculate the the mass values for the grid points at the center of a bin, all we have to do
now is take the arithmetic mean of its two neighboring boundary values, i.e.:
\begin{align*}
  m_i^\text{c}=\frac{m_{i+1}^\text{b}+m_i^\text{b}}{2}
\end{align*}

Thus, after having defined only the three numbers $\mathcal N_m$, $m_\text{min}$, and $m_\text{max}$, it is possible
to interpolate the values of all mass grid points sitting on the boundaries and/or centers of
the bins.

The following figure serves as an illustration of what we just built:
\begin{figure}[h!]
...
the bin index. This will change once the switch to a logarithmic grid is made! For now though,
it is given by
\begin{align*}
